...
Using two pointer technique for window of length ‘cnt’, each time keep track of how many 
...
# Utility function to find minimum swaps required to club all elements less than or
# equals to k together
def minSwap(arr, n, k):
    # find count of elements which are less then or equals to k
    count =0
    for i in range(0,n):
        if (arr[i] <= k):
            count = count + 1
