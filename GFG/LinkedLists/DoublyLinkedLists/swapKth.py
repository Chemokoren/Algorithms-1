"""
A python program to swap kth node from the beginning with kth node from the end
"""
...
class LinkedList:
    ...
    # remove all items in a linked list
    def clearList(self):
        self.head =None
    ...
    def swapKth(self,k):
        ...
        also break self loop if x->next is y or y->next is x
        """
        temp =x.next
        x.next =y.next
        y.next =temp
...
"""
test swaping only one number
"""

print("#################### test swaping only one number #########################")
llist.clearList()
for i in range(1,9):
    llist.push(i)
